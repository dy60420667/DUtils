...
/**
 * 登录页面，只有当用户输入@的时候，才会有下拉列表显示
 * 
 * @author Administrator
 * 
 */
public class EmailAutoCompleteTextViewLogin extends AutoCompleteTextView {
	...
	public EmailAutoCompleteTextViewLogin(Context context) {
		super(context);
	}

	public EmailAutoCompleteTextViewLogin(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}
	...
	private void init() {
		...
		this.setAdapter(new EmailAutoCompleteAdapter(mContext,
				R.layout.layout_item_text, emailSufixs));
		// 使得在输入1个字符之后便开启自动完成
		this.setThreshold(1);
		this.setOnFocusChangeListener(new OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				if (hasFocus) {
					String text = EmailAutoCompleteTextViewLogin.this.getText()
							.toString();
					// 当该文本域重新获得焦点后，重启自动完成
					if (!"".equals(text))
					...
					// EmailAutoCompleteTextView ev =
					// (EmailAutoCompleteTextView) v;
					...
					// {
					// } else {
					// Toast toast = Toast.makeText(mContext, "邮件地址格式不正确",
					// Toast.LENGTH_SHORT);
					// toast.show();
					// }
				}
	...
	protected void performFiltering(CharSequence text, int keyCode) {
		...
		if (index == -1) {
			...
			if(isOver(t.substring(index, t.length()))){
	...
	private boolean isOver(String substring) {
		for(int i=0;i<emailSufixs.length;i++){
			if(emailSufixs[i].equals(substring)){
				return true;
			}
		}
		return false;
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (v == null)
				v = LayoutInflater.from(getContext()).inflate(
						R.layout.layout_item_text, null);
			TextView tv = (TextView) v.findViewById(R.id.tv);
			String t = EmailAutoCompleteTextViewLogin.this.getText().toString();
			int index = t.indexOf("@");
			if (index != -1)
				t = t.substring(0, index);
